...
#ifdef CMD_DATA_SIZE
int cmd_get_data_size64(char* arg, int default_size)
{
...
// 	addr = dp_last_addr;
	...
	if ((flag & CMD_FLAG_REPEAT) == 0) {
		 ...
		 */
		if ((size = cmd_get_data_size64(argv[0], 8)) < 0)
			return 1;
	...
	{
		/* Print the lines. */
		print_buffer64(addr, addr, size, length, DISP_LINE_LEN/size);
		addr += size*length;
	}
#endif
...
int do_mem_mw64 ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	*/
	if ((size = cmd_get_data_size64(argv[0], 8)) < 1)
		return 1;
...
int do_mem_cmp64 (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	/* Check for size specification.
	*/
	if ((size = cmd_get_data_size64(argv[0], 4)) < 0)
		return 1;
...
U_BOOT_CMD(
	...
	"memory write (fill)",
	"[.b, .w, .l, .d] address value [count]"
);

U_BOOT_CMD(
	cp64,	4,	1,	do_mem_cp64,
	"memory copy",
	"[.b, .w, .l, .d] source target count"
);

U_BOOT_CMD(
	cmp64,	4,	1,	do_mem_cmp64,
	"memory compare",
	"[.b, .w, .l, .d] addr1 addr2 count"
);
#if 0
#ifdef CONFIG_CMD_CRC32
...
U_BOOT_CMD(
	...
	"[.b, .w, .l, .d] address number_of_objects"
);
...
U_BOOT_CMD(
	...
	"[.b, .w, .l, .d] address number_of_objects data_to_write"
);
#endif /* CONFIG_LOOPW */
#if 0
U_BOOT_CMD(
...
U_BOOT_CMD(
	...
	"[.b, .w, .l, .d] address count delay(ms)"
);

U_BOOT_CMD(
	mwc64,	4,	1,	do_mem_mwc64,
	"memory write cyclic",
	"[.b, .w, .l, .d] address value delay(ms)"
);
#endif /* CONFIG_MX_CYCLIC */
