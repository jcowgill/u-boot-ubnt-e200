...
void board_mdio_init(void)
{
	...
	smi_clk.cn50xx.phase = (gd->ogd.cpu_clock_mhz + 7) / 7;
...
int early_board_init(void)
{
	...
	if (addr >= 0) {
		...
		gd->ogd.board_desc.board_type = CVMX_BOARD_TYPE_BBGW_REF;
	...
	gd->ogd.ddr_clock_mhz = 266;
	gd->ogd.ddr_ref_hertz = CN5020_FORCED_DDR_AND_CPU_REF_HZ;
	...
	if (addr >= 0) {
		...
		gd->ogd.mac_desc.mac_addr_base[3] =
		    (gd->ogd.board_desc.rev_major << 4) | gd->ogd.
		...
		if (!(env_str = getenv("octeon_mac_base")) ||
		    !ether_aton(env_str,
				(uint8_t *) (gd->ogd.mac_desc.mac_addr_base))) {
			/* Special value allows silent overrides */
			gd->ogd.mac_desc.mac_addr_base[0] = 0x00;
			gd->ogd.mac_desc.mac_addr_base[1] = 0xba;
			gd->ogd.mac_desc.mac_addr_base[2] = 0xdd;
		}
	}
