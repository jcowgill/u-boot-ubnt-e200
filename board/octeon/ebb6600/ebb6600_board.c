...
int board_fixup_fdt(void)
{
	...
	if (mio_qlm_cfg.s.qlm_spd == 15) {
		...
		fdt_key = "1,none";	/* Not Ethernet */
	}
	octeon_fdt_patch(working_fdt, fdt_key, NULL);

	/* Trim the 2nd compact flash slot on revision 1.x boards */
	if (gd->ogd.board_desc.rev_major == 1) {
		debug("%s: Trimming compact flash slot\n", __func__);
		octeon_fdt_patch(working_fdt, "0,slot0", "cavium,cf-trim");
	}
	return 0;
}
...
int early_board_init(void)
{
	...
	octeon_board_get_clock_info(EBB6600_DEF_DRAM_FREQ);

	octeon_board_get_descriptor(CVMX_BOARD_TYPE_EBB6600, 1, 0);

	/* CN66XX has a fixed 50 MHz reference clock */
	...
	if (gd->ogd.ddr_clock_mhz < 100 || gd->ogd.ddr_clock_mhz > 2000) {
		gd->ogd.ddr_clock_mhz = EBB6600_DEF_DRAM_FREQ;
	}
