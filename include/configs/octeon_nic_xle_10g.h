...
/*
 * The NIC XLE 10G does not use the internal arbiter in Octeon.
...
/* DTT sensor
 * The NIC XLE 10G board has a TI TMP421 temperature sensor.
 *
 * - range -55C to +127C
 ...
 * - local temp sensor enabled, min set to 0 deg, max set to 70 dg
...
#include "octeon_nic_xle_4g_shared.h" /* 10g uses same DRAM config as 4g */
