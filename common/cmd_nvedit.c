...
SPI_FLASH|NVRAM|MMC|FAT|REMOTE|RAM} or CONFIG_ENV_IS_NOWHERE
#endif
...
env_set_hook_rc_t setenv_board(const char *var, const char *old_value,
...
int _do_env_set(int flag, int argc, char * const argv[])
{
	...
	if ((argc < 3) || argv[2] == NULL) {
		if (!in_hook) {
			in_hook = 1;
			hook_rc = setenv_board(name, ep ? ep->data : NULL,
					       NULL);
			switch (hook_rc) {
			default:
				break;
			...
			hook_rc = setenv_arch(name, ep ? ep->data : NULL,
					NULL);
			switch (hook_rc) {
			default:
				break;
	...
	if (!in_hook) {
		in_hook = 1;
		hook_rc = setenv_board(name, ep ? ep->data : NULL, value);
		switch (hook_rc) {
		case ENV_NORMAL:
			break;
		case ENV_NO_REPLACE:
		...
		hook_rc = setenv_arch(name, ep ? ep->data : NULL, value);
		switch (hook_rc) {
		case ENV_NORMAL:
			break;
		case ENV_NO_REPLACE:
