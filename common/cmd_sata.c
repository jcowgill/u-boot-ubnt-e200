...
int __sata_initialize(void)
{
	...
	for (i = 0; i < CONFIG_SYS_SATA_MAX_DEVICE; i++) {
		...
		rc = init_sata(i);
		if (rc) {
			memset(&sata_dev_desc[i], 0,
			       sizeof(struct block_dev_desc));
			debug("Error initializing sata device %d\n", i);
			rc = 0;
			continue;
		}
		rc = scan_sata(i);
		if (rc) {
			...
			if (sata_curr_device == i)
				sata_curr_device = -1;
		}
		if ((sata_dev_desc[i].lba > 0) && (sata_dev_desc[i].blksz > 0))
			init_part(&sata_dev_desc[i]);
		if (!rc && first_found_device == -1)
			first_found_device = i;
	}
	if (sata_curr_device == -1)
		sata_curr_device = first_found_device;
	return rc;
}
