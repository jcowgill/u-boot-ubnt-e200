...
#ifdef CONFIG_OCTEON
			addr = virt_to_phys(&qtd[qtd_counter]);
			debug("%s: Setting NULL req *tdp to 0x%llx, len: %d\n",
			      __func__, (u64)addr, left_length);
			*tdp = cpu_to_hc32((uint32_t)addr & 0xffffffff);
#else
			*tdp = cpu_to_hc32((uint32_t)&qtd[qtd_counter]);
#endif
			tdp = &qtd[qtd_counter++].qt_next;
...
#ifdef CONFIG_OCTEON
	addr = virt_to_phys(&qh_list);
	ehci_writel(&hcor->or_ctrldssegment, (addr >> 16) >> 16);
	ehci_writel(&hcor->or_asynclistaddr, addr & 0xffffffff);
#else
