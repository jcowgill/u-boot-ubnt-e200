...
#if defined(CONFIG_OCTEON) && (OCTEON_FIRST_PCIE_BUSNO == 0)
	pci_hose_write_config_byte(hose, dev, PCI_PRIMARY_BUS,
				   PCI_BUS(dev) - hose->first_busno + 1);
	pci_hose_write_config_byte(hose, dev, PCI_SECONDARY_BUS,
				   sub_bus - hose->first_busno + 1);
#else
	pci_hose_write_config_byte(hose, dev, PCI_PRIMARY_BUS,
...
int pciauto_config_device(struct pci_controller *hose, pci_dev_t dev)
{
	...
	case PCI_CLASS_BRIDGE_PCI:
		...
		/* Some devices require a delay after initializing the bridge */
		mdelay(200);	/* Need really long delay for siI3124 RAID controller */
		/*
