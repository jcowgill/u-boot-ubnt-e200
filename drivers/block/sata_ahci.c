...
static int get_ahci_device_data(u8 port, sata_fis_d2h *fis, int fis_len,
				u8 *buf, int buf_len)
{
 ...
 */
static int ahci_sata_rw_cmd(int dev, lbaint_t start, lbaint_t blkcnt, 
			    u8 *buffer, int is_write, int is_lba48)
{
	...
	if (is_lba48) {
		if (is_write)
			fis.command = ATA_CMD_WRITE_EXT;
		else
			fis.command = ATA_CMD_READ_EXT;
	} else {
		if (is_write)
			fis.command = ATA_CMD_WR_DMA;
		else
			fis.command = ATA_CMD_RD_DMA;
	}

	if (is_lba48) {
		fis.lba_high_exp = (block >> 40) & 0xff;
		fis.lba_mid_exp = (block >> 32) & 0xff;
		fis.lba_low_exp = (block >> 24) & 0xff;
	}
	fis.lba_high = (block >> 16) & 0xff;
	fis.lba_med = (block >> 8) & 0xff;
	fis.lba_low = block & 0xff;
	if (is_lba48)
		fis.device = ATA_LBA;
	else
		fis.device = ((block >> 24) & 0x0f) | ATA_LBA;
	fis.sector_count_exp = (blkcnt >> 8) & 0xff;
	fis.sector_count = blkcnt & 0xff;
	
	/* Read from ahci */
	if (get_ahci_device_data(dev, (u8 *) &fis, 20,
		      ...
		      void *buffer, int is_write)
{
	...
	is_lba48 = probe_ent->port[port].is_lba48;
	...
	do {
		...
		if (ahci_sata_rw_cmd(dev, start, num_blks, addr,
				     is_write, is_lba48))
				return blkcnt - blks;
